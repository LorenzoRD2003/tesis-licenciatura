...
Una forma compacta de expresar la idea central de validación en \eUTxO (como se presenta en el artículo) es: $\validator(\textsf{valor}, \datum, \redeemer, \txcontext) = \True$ donde $\validator$ es el validador, $\datum$ es el datum, $\redeemer$ es el redeemer, y $\txcontext$ es la transacción que gasta la salida (o un contexto derivado de ella).
...
Esto permite diseñar mecanismos de verificación más eficientes, tanto en términos de tamaño de datos como de costo computacional \Onchain, lo cual resulta especialmente relevante en el contexto de las restricciones del modelo \eUTxO. El protocolo \Mithril será analizado en profundidad en el Capítulo 7.
