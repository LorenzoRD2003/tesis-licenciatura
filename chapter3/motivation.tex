...
A nivel de libro contable, \Cardano sigue un paradigma \UTxO (\emph{Unspent Transaction Output}, como \Bitcoin): las transacciones consumen \textit{outputs} previamente no gastados y crean nuevos \textit{outputs} que podrán gastarse en el futuro. Cada \UTxO puede consumirse \emph{una sola vez y en su totalidad}. \Cardano extiende este modelo mediante \eUTxO, permitiendo que los \textit{outputs} lleven datos y que las direcciones estén respaldadas por \textit{scripts}, lo que habilita contratos inteligentes expresivos sin adoptar un modelo global de estado completamente basado en cuentas (como el de \Ethereum).
...
Para un \zkBridge, el determinismo es valioso porque los \Bridge operators pueden precomputar costos y determinar el éxito \Offchain. El determinismo también conlleva parámetros concretos del protocolo que limitan el cómputo y los datos permitidos por transacción/bloque, analizados en la Sección~\ref{sec:motivation-limites-del-protocolo-y-su-importancia-en-un-zkbridge}.

\section{El modelo e\UTxO}
\label{sec:motivation-el-modelo-e-utxo}

\subsection{De \UTxO a \eUTxO: validadores, datums, redeemers, contexto}
\label{subsec:motivation-de-utxo-a-eutxo-validadores-datums-redeemers-contexto}

En el modelo \UTxO tradicional, un \textit{output} es principalmente un par \emph{(dirección, valor)}. Gastar un \textit{output} requiere proporcionar el witness necesario para desbloquear esa dirección (usualmente una firma). \Cardano generaliza esto en dos direcciones clave: las direcciones pueden incorporar lógica arbitraria (scripts), y los \textit{outputs} pueden transportar datos específicos interpretables por los scripts.
...
\begin{itemize}[noitemsep]
	\item \textbf{Transporte de estado:} los \textit{outputs} incluyen un datum $\delta$ para almacenar datos específicos del contrato (estado).
...
\label{subsec:motivation-maquinas-de-estado-en-eutxo}
...
\label{subsec:motivation-caracteristicas-de-eutxo-relevantes-para-aplicaciones-complejas}
...
\label{sec:motivation-smart-contracts-en-cardano-plutus-y-aiken}
\subsection{Plutus, Plutus Core y PlutusTx}
\label{subsec:motivation-plutus-plutus-core-y-plutustx}
...
\label{sec:motivation-limites-del-protocolo-y-su-importancia-en-un-zkbridge}

\subsection{Contabilización de recursos: unidades de ejecución, determinismo y validación en dos fases}
\label{subsec:motivation-contabilizacion-de-recursos-unidades-de-ejecucion-determinismo-y-validacion-en-dos-fases}
...
\label{subsec:motivation-limites-de-tamano-y-computo}
...
\begin{table}[t]
	...
	\begin{tabular}{p{0.22\linewidth} p{0.46\linewidth} p{0.20\linewidth}}
		...
		\maxTxExUnits & Límite de \ExUnits por $\tx$ &
		\begin{tabular}[t]{@{}l@{}}
			memoria: \texttt{1.4e7} \\
			CPU: \texttt{1e10}
		\end{tabular} \\
		\maxBlockExUnits & Límite de \ExUnits por block &
		\begin{tabular}[t]{@{}l@{}}
			memoria: \texttt{5e7} \\
			CPU: \texttt{4e10}
		\end{tabular} \\
...
\begin{enumerate}[noitemsep]
		\item Un sistema de pruebas más amigable para verificadores auxiliares acotados. En esta tesis, \Groth se usa en las relaciones de pertenencia a \textit{snapshots} y actualización del \SMT anti-replay, desarrolladas respectivamente en las Secciones~\ref{sec:tx-snapshot-membership-el-circuito-de-pertenencia-a-una-transactionsnapshot} y~\ref{sec:tx-set-update-el-circuito-txsetupdatecircuit}, debido a su verificación eficiente y tamaño de prueba constante.
		\item Reducir el trabajo \Onchain usando proof recursion o proof aggregation.
		\item Dividir la verificación en múltiples transacciones cuando el objeto criptográfico completo no entra en presupuesto. Este es el caso de los certificados \Mithril, cuya verificación \STM sigue el camino \HaloTwo/\Midnight y requiere la partición en dos fases de la Sección~\ref{subsec:mithril-a-cardano-particion-de-la-verificacion-en-dos-transacciones}.
\end{enumerate}

\section{Praos y el modelo operativo de proof-of-stake}
\label{sec:motivation-praos-y-el-modelo-operativo-de-proof-of-stake}
\subsection{Proof of stake, stake pools y \SPOs}
\label{subsec:motivation-proof-of-stake-stake-pools-y-spos}
...
\label{subsec:motivation-estructura-temporal-slots-y-epocas}
...
En el paper de \Praos, la elección de líderes se describe como una \emph{prueba local privada} utilizando una \VRF (\textit{Verifiable Random Function}, o función aleatoria verificable) sobre el timestamp actual y un \Nonce específico de la época. Solo la parte elegida sabe que es líder hasta que produce un bloque que contiene la prueba.
...
\label{subsec:motivation-eleccion-de-lideres-mediante-vrf}
...
\label{subsec:motivation-kes-y-certificados-operativos}
...
\label{subsec:motivation-light-clients-en-cardano}

Una dificultad fundamental en el diseño de \Bridges sobre \Cardano es la obtención de una representación verificable del estado de la blockchain sin volver a ejecutar completamente el protocolo de consenso. Es decir, necesitamos usar \LightClients, conforme al modelo presentado en la Sección~\ref{subsec:bridges-modelos-de-seguridad-de-bridges}.

En esta tesis, \Mithril no se considera un \LightClient en sentido pleno. Se utiliza como una interfaz de autenticación que cumple, para las entidades certificadas, la función que el modelo abstracto del bridge asigna al \LightClient: permitir que la cadena destino verifique evidencia compacta sobre distribuciones de stake y conjuntos de transacciones. Dentro del ecosistema de \Cardano, \Mithril implementa esta función mediante firmas agregadas ponderadas por stake y certificados compactos \cite{cryptoeprint:2021/916}. Esta interfaz no verifica los headers de \Praos, no ejecuta la regla de selección de cadena ni establece por sí sola la finalidad de una transacción.

Esto permite diseñar mecanismos de verificación más eficientes, tanto en términos de tamaño de datos como de costo computacional \Onchain, lo cual resulta especialmente relevante en el contexto de las restricciones del modelo \eUTxO. El protocolo \Mithril se analiza en profundidad en el Capítulo~\ref{chap:mithril}.

\section{Direcciones futuras}
\label{sec:motivation-direcciones-futuras}

\subsection{Peras}
\label{subsec:motivation-peras}
...
\label{sec:motivation-conclusiones}

En conjunto, el modelo \eUTxO, los límites estrictos de recursos y el protocolo de consenso de \Cardano definen un espacio de diseño altamente restringido pero bien estructurado. Estas características condicionan fuertemente las decisiones del \zkBridge, desarrolladas en el Capítulo~\ref{chap:decisiones-de-diseno} después de introducir la capa de certificación de \Mithril.
