...
Los polinomios constituyen una de las herramientas algebraicas más versátiles en matemática y criptografía. Permiten representar funciones, modelar relaciones algebraicas y, como veremos más adelante, describir cómputos completos. En esta sección introducimos el anillo de polinomios y algunas de sus propiedades fundamentales.
...
Aunque los polinomios se definen como expresiones formales, resulta natural asociarles una función mediante evaluación, es decir $P: \K \to \K$. En cuerpos finitos, esta distinción es particularmente relevante: polinomios distintos en $\poly{\Fp}$ pueden inducir la misma función.
...
\begin{theorem}
	Sea $p \in N$ primo, y sean $P, Q \in \poly{\Fp}$ dos polinomios de grado a lo sumo $n$. Se toma un $k \in \Fp$ de forma uniformemente aleatoria. Si $P \neq Q$, entonces
	$$\Prob[P(k) = Q(k)] \leq \frac{n}{p}.$$
\end{theorem}
...
\begin{theorem}[Existencia y unicidad del polinomio interpolador]
	Existe un único polinomio interpolador $P^*$ con $\degp P^* = n$ para un conjunto $D$ de $n + 1$ puntos, y está dado por:
...
La interpolación sobre cuerpos finitos desempeña un rol central en construcciones criptográficas modernas, en particular en sistemas de pruebas sucintas. Utilizaremos esto durante el capítulo 2.
