...
\label{def:apendice-a-curvas-elipticas-curva-eliptica}
...
\label{def:apendice-a-curvas-elipticas-definicion-geometrica-de-suma-en-curvas-elipticas}
...
\label{def:apendice-a-curvas-elipticas-definicion-algebraica-de-suma-en-curvas-elipticas}
...
\label{def:apendice-a-curvas-elipticas-orden-de-un-punto-en-una-curva-eliptica}
...
\label{def:apendice-a-curvas-elipticas-ecdlp}
...
\subsection{Representación y compresión de puntos}
\label{subsec:ec-point-compression}

Para transmitir o almacenar un punto de una curva elíptica es necesario convertirlo en una secuencia de bytes. Una representación directa de un punto afín $P=(x,y)$ incluye ambas coordenadas. Sin embargo, en una curva dada por $y^2=x^3+ax+b$, la coordenada $x$ determina como máximo dos valores posibles para $y$, que corresponden a puntos opuestos. Una representación comprimida puede entonces almacenar $x$ junto con un bit que indique cuál de esos dos valores debe elegirse, mientras que el punto en el infinito utiliza una codificación distinguida.

En este contexto, ``compresión'' significa utilizar una serialización reversible de menor tamaño. La descompresión interpreta los bytes, reconstruye la coordenada faltante y rechaza representaciones inválidas o no canónicas. Dependiendo de la interfaz criptográfica, también puede comprobar que el punto pertenezca al subgrupo esperado. Sólo después de esta conversión el punto puede utilizarse en operaciones de grupo como sumas o multiplicaciones escalares. Esta frontera entre bytes serializados y puntos de curva aparece de forma explícita en las interfaces criptográficas utilizadas posteriormente en el proyecto.

\subsection{Curvas elípticas útiles para este trabajo}
\label{subsec:apendice-a-curvas-elipticas-curvas-elipticas-utiles-para-este-trabajo}

\begin{itemize}[noitemsep]
	\item \textbf{\BLS.} Es la curva emparejable más importante para esta tesis. Aparece en la discusión de \Groth y de \KZG, en la motivación de las decisiones de diseño y en la implementación \Onchain del verificador, ya que \Cardano expone primitivas criptográficas sobre \BLS \cite{CIP0381} y bibliotecas como \texttt{ak-381} se apoyan precisamente en esa curva. Además, en el bloque de circuitos del capítulo de implementación, el cuerpo escalar asociado a \BLS se reutiliza para modelar nodos internos del \SMT y para formalizar circuitos basados en \PoseidonHash.
