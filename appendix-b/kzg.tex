...
\label{subsec:apendice-b-kzg-definicion-formal-e-intuicion}

\begin{definition}[\KZG \PCS]
\label{def:apendice-b-kzg-kzg-pcs}
...
Para relacionarlo con la Definición~\ref{def:apendice-b-commitment-schemes-pcs}, observamos la instancia específica utilizada para cada uno de los conjuntos:
...
\label{subsec:apendice-b-kzg-trusted-setup}
...
\subsection{Aperturas por conjuntos de puntos en \texorpdfstring{\HaloTwoMultiOpen}{Halo2 Multi-Open}}
\label{subsec:kzg-halo2-multiopen}

La construcción anterior organiza las aperturas a partir de cada punto de evaluación: primero combina los polinomios consultados en un mismo punto y luego agrega las ecuaciones resultantes. Otra estrategia, utilizada por las bibliotecas de \Midnight que sostienen el circuito \MidnightSTMCircuit \cite{MithrilSTMRepo}, agrupa los polinomios que comparten un mismo \emph{conjunto} de puntos de evaluación. Esta diferencia no cambia el compromiso \KZG subyacente, pero sí modifica la forma algebraica del argumento de apertura y el trabajo que debe reconstruir el \textit{verifier}.

Consideremos $r$ conjuntos de puntos. Para $0 \leq j < r$, escribimos
$$S_j=\{z_{j,0},\ldots,z_{j,m_j-1}\}$$
y denotamos por $p_{j,i}(X) \in \poly{\Fp}_{\leq d}$ a los polinomios asociados al conjunto $S_j$. Sean además $x_1,x_2,x_3,x_4 \in \Fp$ desafíos escalares del protocolo. En una ejecución no interactiva, estos valores se derivan del \transcript mediante la transformación de \FiatShamir de la Sección~\ref{sec:aritmetizacion-y-construccion-de-un-snark-protocolo-fiatshamir-y-no-interactividad}; por ahora sólo importa su función como coeficientes aleatorios de agregación.

Para cada conjunto, el \textit{verifier} forma la combinación lineal
$$Q_j(X)=\sum_i x_1^i\,p_{j,i}(X) \in \poly{\Fp}_{\leq d}.$$
Luego debe reconstruir el valor de $Q_j$ en el punto adicional $x_3$ a partir de las evaluaciones conocidas sobre todo $S_j$. Para ello define
$$R_j(x_3) = \sum_{\ell = 0}^{m_j - 1}\lambda_{j,\ell}(x_3) \, Q_j(z_{j, \ell}), \qquad Z_j(X) = \prod_{\ell = 0}^{m_j - 1}(X - z_{j, \ell}),$$
donde $\lambda_{j,\ell}(x_3)$ son los coeficientes de interpolación de Lagrange y $Z_j$ es el polinomio que se anula en los puntos de $S_j$. A diferencia de verificar una apertura \KZG independiente por cada evaluación, el argumento contiene un único elemento de grupo $\Proof$ y valores auxiliares $q_j(x_3)$. Con esos datos, el \textit{verifier} reconstruye
$$f(x_3) = \sum_{j=0}^{r-1} x_2^j\cdot \frac{q_j(x_3)-R_j(x_3)}{Z_j(x_3)}.$$

Finalmente, agrega las contribuciones de los distintos conjuntos con potencias de $x_4$ y forma
$$\mathrm{final\_com}=\sum_{j=0}^{r-1}x_4^j Q_j(\tau)G_1+x_4^{r}f(\tau)G_1, \qquad v = \sum_{j=0}^{r-1}x_4^j q_j(x_3)+x_4^{r}f(x_3).$$
La verificación termina en una única ecuación de pairing:
$$\pairing{\Proof}{\tau G_2} \stackrel{?}{=} \pairing{\mathrm{final\_com} - vG_1 + x_3\Proof}{G_2}.$$

La diferencia esencial con \GWCNineteen está en el lugar donde se realiza la agregación. \GWCNineteen combina aperturas \KZG organizadas por punto de evaluación; \HaloTwoMultiOpen mueve parte del trabajo al proceso de reconstruir una apertura común para cada conjunto de puntos, mediante interpolaciones y polinomios de anulación. Ambos caminos conservan una verificación final sucinta, pero inducen estructuras de datos, acumuladores y costos de verificación diferentes. Esta distinción es la que impide trasladar mecánicamente un verificador generado para el primer protocolo al circuito de \Mithril basado en el segundo.

\subsection{Lo que tenemos hasta ahora}
\label{subsec:apendice-b-kzg-lo-que-tenemos-hasta-ahora}

Las dos estrategias de apertura múltiple anteriores muestran cómo la linealidad de \KZG permite condensar muchas evaluaciones en una ecuación final de pairing, aunque organicen esa agregación de manera diferente. Este refinamiento no altera el papel que los compromisos polinomiales cumplen dentro de la construcción general. En la sección de \QAP mostramos que la verificación de un circuito aritmético puede reformularse como una identidad entre polinomios de la forma
$$AB - C = HZ,$$
...
\item No interactividad: \KZG asume desafíos enviados por el verifier (en concreto, el punto $z \in \Fp$). Para eliminar esta interacción, necesitaremos la transformación de \FiatShamir de la Sección~\ref{sec:aritmetizacion-y-construccion-de-un-snark-protocolo-fiatshamir-y-no-interactividad}.
