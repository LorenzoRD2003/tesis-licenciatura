...
\begin{definition}[\SNARK, \zkSNARK]
	...
	\begin{itemize}
		...
		Para todo prover probabilístico en tiempo polinomial $\Prover^\star$, existe un extractor probabilístico en tiempo polinomial $\Extractor^{\Prover^\star}$ tal que, para toda instancia $x \in \InstanceSpace$, si
	...
	\begin{itemize}
		\item Conocimiento cero: Para todo $x \in \InstanceSpace$, existe un simulador probabilístico en tiempo polinomial $\Simulator(\CRS,x)$ tal que la distribución de las pruebas simuladas $\Simulator(\CRS,x)$ es computacionalmente indistinguible de la distribución de pruebas reales $\Prover_\CRS(x,w)$, para cualquier $w \in \WitnessSpace$ tal que $(x,w) \in \Relation$. Intuitivamente, esto implica que el verifier podría haber generado por sí mismo una prueba indistinguible de la real, sin conocer el witness.
	\end{itemize}
\end{definition}
