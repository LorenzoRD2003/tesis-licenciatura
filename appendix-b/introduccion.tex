...
Intuitivamente, el \textit{witness} es la información privada que posee el prover y que hace verdadera la afirmación pública que quiere demostrar. La instancia pública, en cambio, contiene los datos que el verifier sí conoce. Así, una prueba \ZK no busca revelar el \textit{witness}, sino convencer al verifier de que existe un \textit{witness} válido para la instancia pública considerada. Cuando el protocolo es interactivo, llamamos \textit{transcript} a la secuencia ordenada de mensajes intercambiados entre prover y verifier durante una ejecución concreta del protocolo.

\begin{definition}[Protocolo de pruebas \ZK]
\label{def:apendice-b-introduccion-protocolo-de-pruebas-zk}
...
\begin{itemize}[noitemsep]
	\item Si $\Prover$ ejecuta su algoritmo con un $w$ tal que $(x,w) \in \Relation$, porque efectivamente \textbf{conoce} un $w$ que cumple esto, entonces seguramente podrá convencer a $\Verifier$ de que lo conoce.
		...
		\item En esta introducción estamos trabajando con la noción de \textit{honest-verifier zero knowledge}. Esto significa que $\Verifier$ no aprende ninguna información esencial sobre $w$ a partir del \textit{transcript} del protocolo, más allá de la validez de la afirmación, siempre que siga honestamente el protocolo. La propiedad se enuncia mediante un simulador: un algoritmo que, sin conocer un \textit{witness} válido, puede generar una vista del protocolo computacionalmente indistinguible de una ejecución real para ese verifier honesto. Intuitivamente, si existe tal simulador, entonces todo lo que ve el verifier durante el protocolo podría haberse generado sin conocer el \textit{witness}; por lo tanto, la interacción no revela información adicional sobre $w$. Además, el simulador debe ser eficiente: si generar la vista simulada requiriera mucho más tiempo que una ejecución real, ese costo también podría servir para distinguir ambas distribuciones.
...
\label{def:apendice-b-introduccion-esquema-de-computo-verificable-zk}
	...
	\begin{enumerate}[noitemsep]
		...
		\item Eficiencia del verifier: El tiempo de ejecución de $\Verifier$ es polinomial en el tamaño de la entrada $(x, y)$, y sublineal respecto al costo requerido para computar directamente $f(x,w)$.
	\end{enumerate}
