...
Esta sección presenta la transformación que elimina la interacción en protocolos de desafío público y muestra su aplicación al esquema \KZG.

\subsection{De protocolos interactivos a no interactivos}
\label{subsec:apendice-b-fiat-shamir-de-protocolos-interactivos-a-no-interactivos}

Los protocolos de argumentos de conocimiento suelen ser interactivos entre el prover y el verifier. Sin ir más lejos, el argumento sucinto \KZG de la Sección~\ref{sec:aritmetizacion-y-construccion-de-un-snark-kzg} requiere que el verifier envíe un valor aleatorio como challenge al prover.
...
\label{def:apendice-b-fiat-shamir-sigmaprotocol}
	...
	\begin{enumerate}[itemsep=0.1 cm]
		...
		\item El verifier responde con un desafío uniformemente aleatorio $\Challenge$.
		\item El prover le envía una respuesta $\pi = \Open(c, \Challenge, w, x; r)$.
	\end{enumerate}
	Una ejecución completa del $\Sigma$-protocol produce la tripla $(c, \Challenge, \pi)$. El verifier acepta la respuesta $\pi$ si $\Verify(x, c, \Challenge, \pi) = 1$.
\end{definition}

\begin{definition}[Transformación \FiatShamir]
\label{def:apendice-b-fiat-shamir-transformacion-fiatshamir}
	Sea $\Pi = (\Commit, \Open, \Verify)$ un \SigmaProtocol para una relación $\Relation \subseteq \InstanceSpace \times \WitnessSpace$, y supongamos que su espacio de desafíos es compatible con el \textit{output} de una función hash criptográfica $\Hash: \Strings{*} \to \Strings{n}$. La \emph{transformación \FiatShamir} es un protocolo no interactivo $\Pi_{FS} = (\Prove_{FS}, \Verify_{FS})$ definido como:
	\begin{itemize}[noitemsep]
		\item $\Prove_{FS}(x,w)$, ejecutado por el prover:
		\begin{itemize}[noitemsep]
			\item Se computa $c = \Commit(x,w;r)$.
			...
			\item Se computa la respuesta $\pi = \Open(c, \Challenge, w, x; r)$.
		\end{itemize}
		\item $\Verify_{FS}(x,c,\pi)$, ejecutado por el verifier:
		\begin{itemize}[noitemsep]
			\item Se recomputa el challenge $\Challenge = \Hash(x \concat c)$.
			\item Se acepta si y solo si $\Verify(x,c,\Challenge,\pi) = 1$.
		\end{itemize}
	\end{itemize}
\end{definition}

\subsection{Transformación \FiatShamir en \KZG}
\label{subsec:apendice-b-fiat-shamir-transformacion-fiatshamir-en-kzg}
...
Sea una función hash criptográfica $\Hash: \Strings{*} \to \Fp$, modelada como oráculo aleatorio en el \ROM. La transformación consiste en reemplazar el desafío aleatorio $\Challenge \in \Fp$ por $\Challenge := \Hash(\textnormal{encode}(c))$, donde $c$ es el compromiso polinomial generado por el prover. En implementaciones reales, la entrada al hash incluye una codificación determinística y no ambigua del compromiso, así como cualquier otro contexto público que deba formar parte del transcript.
...
\begin{algorithm}[ht]
	...
	\begin{algorithmic}[1]
		\Require Polinomio $P \in \poly{\Fp}_{\leq d}$, \SRS $\{G_1, \tau G_1, \ldots, \tau^d G_1, G_2, \tau G_2\}$.
		...
		\State $\Challenge \gets \Hash(\textnormal{encode}(c))$ \Comment{cómputo del \textit{challenge} $\Challenge$}
...
\begin{algorithm}[ht]
	...
	\begin{algorithmic}[1]
		\Require Compromiso polinomial $c \in \GOne$, evaluación $y \in \Fp$, argumento de evaluación $\pi \in \GOne$, y los elementos $G_2, \tau G_2 \in \GTwo$ de la \SRS.
		...
		\State $\Challenge \gets \Hash(\textnormal{encode}(c))$ \Comment{recómputo del \textit{challenge} $\Challenge$}
...
En el caso del esquema \KZG, el desafío $\Challenge$ pasa de ser generado por el verificador a determinarse como el resultado de una función hash aplicada al compromiso. El esquema \Groth de la Sección~\ref{sec:aritmetizacion-y-construccion-de-un-snark-groth}, en cambio, se define directamente como un argumento no interactivo en el modelo CRS y no se obtiene aplicando \FiatShamir al protocolo anterior. Su $\Setup$ genera parámetros públicos a partir de secretos que deben descartarse, mientras que cada ejecución de $\Prove$ emplea aleatoriedad fresca $r,s$ para randomizar la prueba y alcanzar conocimiento cero; ninguna de esas dos fuentes de aleatoriedad desempeña el papel de un desafío derivado mediante hash. La transformación de \FiatShamir reaparece además en el \transcript de la verificación \HaloTwo discutido en la Subsección~\ref{subsec:mithril-a-cardano-validez-del-transcript-y-binding-entre-fases}.

\subsection{Limitaciones}
\label{subsec:apendice-b-fiat-shamir-limitaciones}
