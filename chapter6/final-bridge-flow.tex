...
\label{sec:final-bridge-flow-flujo-final-del-zkbridge}

Esta sección describe el flujo operacional completo que une los validadores de la Sección~\ref{sec:validadores-y-flujo-transaccional-formalizacion-de-los-validadores-principales}, la adaptación de \HaloTwo de la Sección~\ref{sec:mithril-a-cardano-del-circuito-halotwo-de-mithril-a-cardano} y los circuitos auxiliares de las Secciones~\ref{sec:tx-set-update-el-circuito-txsetupdatecircuit} y~\ref{sec:tx-snapshot-membership-el-circuito-de-pertenencia-a-una-transactionsnapshot} en un protocolo transaccional ejecutable. En particular, explica cómo se ensamblan las transacciones, qué ajustes de \textit{tooling} fueron necesarios para volver reproducible el flujo y cómo se organizó la validación mediante tests.

\subsection{El ecosistema \texorpdfstring{\TxThree/\Trix/\CShell/\Dolos}{Tx3/Trix/cshell/dolos}}
\label{subsec:final-bridge-flow-el-ecosistema-tx3-trix-cshell-dolos}
...
La Tabla~\ref{tab:bridge-toolchain-versions} resume las versiones de las herramientas usadas durante el desarrollo del \zkBridge y que volvieron operable su lógica criptográfica. \TxThree no tiene una versión, mientras que las herramientas que lo interpretan y ejecutan sí.
...
\label{subsec:final-bridge-flow-estrategia-de-tests-con-aiken-check}
