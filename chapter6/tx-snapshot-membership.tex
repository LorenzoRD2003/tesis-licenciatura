...
\label{sec:tx-snapshot-membership-el-circuito-de-pertenencia-a-una-transactionsnapshot}
...
\label{subsec:tx-snapshot-membership-modelo-matematico-del-transactionsnapshot-pythagoras-de-mithril}
...
Fijamos la función de hash $\BLAKETwoSTwoFiveSix: \Strings{\star} \to \Strings{256}$, y para cada longitud de bytes $\ell \in \N$ escribimos $\Bytes{\ell} = \Strings{8\ell}$. Si $\TxId \in \Bytes{32}$ es el identificador canónico de una transacción de \Cardano según la Definición~\ref{def:txid-canonico-cardano}, la hoja en el \textit{sub-tree} es su codificación hexadecimal ASCII en minúsculas $\mathsf{hex}_{\mathsf{lc}}(\TxId) \in \Bytes{64}$, que luego se hashea con \BLAKETwoSTwoFiveSix a \Bytes{32}.
...
\label{subsec:tx-snapshot-membership-public-inputs-y-witness-del-circuito}
...
\label{subsec:tx-snapshot-membership-la-relacion-np-de-txsnapshotmembershipcircuit}
...
\label{subsec:tx-snapshot-membership-familias-de-restricciones-del-circuito}
...
\begin{enumerate}[noitemsep]
	...
	\item En cada paso tipado, un bit \texttt{kind} determina si el hermano relevante es una cadena de bytes o un hash. Se garantiza además que el slot inactivo quede anulado.
		\item Cuando un paso está deshabilitado, el circuito impone que tanto sus selectores como sus datos sean cero y que el \textit{output} propague exactamente el valor de entrada.
...
\label{subsec:tx-snapshot-membership-vinculo-exacto-con-mintingvalidator}

La utilidad del circuito en el protocolo aparece al mirar la interfaz del \MintingValidator, cuyas condiciones de aceptación fueron formalizadas en la Subsección~\ref{subsec:validadores-y-flujo-transaccional-validador-de-minting-del-zkbridge} y forman parte de las condiciones necesarias de \MintTx. El mapeo entre la matemática del argumento \Groth y la validación \Onchain es el siguiente:
\begin{enumerate}[noitemsep]
...
\label{subsec:tx-snapshot-membership-transicion-experimental-a-lagrange}
...
\end{enumerate}
Y expone exactamente los mismos seis \textit{outputs} públicos que el circuito \textit{legacy}.
