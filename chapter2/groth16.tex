...
\subsection{El protocolo \Groth}

En esta sección presentamos formalmente el argumento de conocimiento subyacente al esquema \Groth en su versión interactiva. Sea $\Relation \subseteq \InstanceSpace \times \WitnessSpace$ una relación en $\NP$. Como en la Sección 2.5, suponemos que la relación ha sido aritmetizada como un \QAP de tamaño $m$ y dimensión $n$ sobre un cuerpo finito $\K = \Fp$, y todo se reduce a argumentar sucintamente una identidad polinomial. Vamos a ligar todos estos nombres de objetos antes de entrar a definir el protocolo propiamente dicho.
...
$$\AssignmentVector = (z_1, \ldots, z_n) = (x_1, \ldots, x_\ell, w_1, \ldots, w_{n-\ell}) \in \Fp^n$$

donde $x_1 = 1$, $\InstanceSpace = \Fp^\ell$ con $x = (x_1, \ldots, x_\ell)$, $\WitnessSpace = \Fp^{n-\ell}$ con $w = (w_1, \ldots, w_{n-\ell})$, definiendo los polinomios agregados

$$A(x) = \sum_{i=1}^n \AssignmentVector_i A_i(x), B(x) = \sum_{i=1}^n \AssignmentVector_i B_i(x), C(x) = \sum_{i=1}^n \AssignmentVector_i C_i(x),$$

y para un polinomio objetivo $Z(x)$ construido sobre $r_1, \ldots, r_m \in \Fp$ se cumple que $(x, w) \in \Relation$ si y solo si existe un polinomio $H(x)$ tal que

$$A(x) \cdot B(x) - C(x) = H(x) Z(x).$$

Además, sean $\GOne, \GTwo, \GT$ grupos cíclicos con orden primo $p$, con un pairing bilineal no degenerado $e: \GOne \times \GTwo \to \GT$.

\begin{definition}[Protocolo interactivo \Groth]
	El \emph{protocolo interactivo \Groth} busca argumentar la existencia de un polinomio $H$ y consta de los siguientes algoritmos:
	...
	\begin{itemize}
		...
		\begin{itemize}[noitemsep]
			...
			\item Para $\ell + 1 \leq i \leq n$ (i.e., los índices correspondientes al witness), los elementos
			...
			\item Los elementos $\left\{\frac{\tau^k Z(\tau)}{\delta}\right\}_{k=0}^{m-2}$.
		\end{itemize}
		...
		\end{enumerate}
		La salida del algoritmo $\Prove$ es la tripla $\pi = (\pi_A, \pi_B, \pi_C) \in (\GOne, \GTwo, \GOne)$.
		\item $\Verify$: El verifier $\Verifier$, que solamente conoce $x$, reconstruye la parte pública $A_{pub}(\tau) = \sum_{i=1}^{\ell} \AssignmentVector_i A_i(\tau)$. Luego, acepta si:
		$$e(\pi_A, \pi_B) = e(\alpha G_1, \beta G_2) \cdot e(A_{pub}(\tau) G_1, \gamma G_2) \cdot e(\pi_C, \delta G_2).$$
	\end{itemize}
\end{definition}

No está demás recordar que \Groth es un argumento de conocimiento, por lo que cumple la propiedad de solidez computacional: es decir que para todo argumento $\pi$ tal que $\Verify(x, \pi) = 1$, existe un extractor probabístico en tiempo polinomial con acceso al prover que devuelve un witness $w$ tal que $(x, w) = 1$.
...
A diferencia de los $\Sigma$-\textit{protocols} discutidos en la sección anterior, \Groth no expone un \textit{challenge} explícito del \textit{verifier}. En cambio, la randomización está embebida en la fase del \textit{trusted setup} mediante la tupla $(\tau, \alpha, \beta, \gamma, \delta)$. \Groth ya es \NIZK por diseño. En particular, no se enmarca en el \ROM, sino en el \AGM (Algebraic Group Model) \cite{Fuchsbauer2018}.
...
$$\begin{aligned}
	\pi_C &= \frac{\beta A(\tau) + \alpha B(\tau) + C(\tau)}{\delta} G_1 - \frac{\beta A_{pub}(\tau) + \alpha B_{pub}(\tau) + C_{pub}(\tau)}{\delta} G_1 \\
...
\begin{itemize}[itemsep=0.1cm]
	...
	\item Dijimos en la sección 2.7 que \KZG por sí solo no era \ZK. La aleatoriedad $r, s$ introducida por \Groth, así como los términos cruzados $(rB(\tau) + sA(\tau) - rs\delta)G_1$ garantizan que la distribución de la prueba sea independiente del witness, y por lo tanto es \ZK.
\end{itemize}
...
\begin{itemize}[noitemsep]
	...
	\item La \SRS tiene una cantidad lineal de elementos en $m$ y $\ell$. En general $l = \Omega(n)$, de modo que a medida que aumenta el tamaño del circuito, aumenta el tamaño del \textit{trusted setup} necesario. El \textit{trusted setup} de \Groth es una fase costosa, y no es universal, sino específica para cada circuito.
\end{itemize}
...
\begin{itemize}[itemsep = 0.1 cm]
	...
	$$\begin{aligned}
		\pi_C &= \sum_{i=1}^{n} \AssignmentVector_i \left(\frac{\beta A_i(\tau) + \alpha B_i(\tau) + C_i(\tau)}{\delta} G_1 \right) + \sum_{j=0}^{m-2} h_j \left(\frac{\tau^j T(\tau)}{\delta} G_1 \right) \\
...
Esta observación es particularmente relevante en el contexto de \Cardano, donde las capacidades criptográficas disponibles onchain determinan directamente qué construcciones \zkSNARK usar. En concreto, existe una implementación de pairings bilineales sobre la curva \BLS en la biblioteca estándar de \Plutus, así como un verificador \Groth hecho por ModuloP \cite{groth_validator_modulop}. Esto, en conjunto con la extrema sucintez del argumento \Groth, determina que sea el \zkSNARK a utilizar durante nuestro proyecto.
