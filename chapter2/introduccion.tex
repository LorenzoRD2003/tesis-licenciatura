...
Una prueba de conocimiento cero (\ZeroKnowledgeProof) o prueba \ZK es un protocolo entre dos partes, en donde una quiere convencer a otra de que posee un valor, sin revelar nada sobre el conocimiento de este valor más allá de la veracidad de la información. Estas partes se conocen como \textbf{Prover} $\Prover$ y \textbf{Verifier} $\Verifier$. Podemos pensar a estas partes como personas ejecutando un algoritmo, o como algoritmos en sí mismos (usaremos estos conceptos de forma indistinta). Esta idea fue introducida inicialmente por Goldwasser, Micali y Rackoff en \cite{DBLP:journals/siamcomp/GoldwasserMR89}. Se trabaja con el modelo \textit{honest-verifier}, que asume que $\Verifier$ actúa en forma honesta. Daremos una definición formal, y luego construiremos intuición sobre las pruebas \ZK.
 
\begin{definition}[Protocolo de pruebas \ZK, prueba \ZK, protocolo (no) interactivo]
	Sea $\Relation \subseteq \InstanceSpace \times \WitnessSpace$ una relación entre dos conjuntos (coloquialmente conocidos como \textbf{\textit{public inputs}} y \textbf{\textit{witness}} respectivamente), y sea $\ProofSpace$ un conjunto (coloquialmente conocido como de \textbf{pruebas}). Un protocolo de pruebas \ZK es un protocolo $(\Prover, \Verifier)$ entre un algoritmo \textit{prover} no necesariamente determinístico
	$$\Prover: \mathcal{X} \times \WitnessSpace \to \Pi,$$
	y un algoritmo verifier determinístico que corre en tiempo polinomial
	$$\Verifier: \InstanceSpace \times \ProofSpace \to \{0,1\},$$
	tales que para cualesquiera $x \in \InstanceSpace, w \in \WitnessSpace$ con $(x, w) \in \Relation$, valen las siguientes tres propiedades:
	\begin{enumerate}[noitemsep]
		\item Completitud: Para $\pi = \Prover(x,w)$, vale $\Prob[\Verifier(x, \pi) = 1 \mid (x,w) \in \Relation] = 1$.
		\item Solidez: Para cualquier prover $\Prover^*$ y $\pi = \Prover^*(x,w)$, vale $\Prob[\Verifier(x, \pi) = 1 \mid (x,w) \not\in \Relation] \leq \epsilon$, donde $\epsilon$ es un valor negligible.
		\item Conocimiento cero: Para cualquier $x \in \InstanceSpace$, existe un algoritmo simulador eficiente $\Simulator_x$ que puede generar una secuencia de interacciones con $\Verifier$ que le sea indistinguible en distribución de probabilidad con respecto a una interacción real con $\Prover$.
	\end{enumerate}
	Las probabilidades se toman sobre todas las posibles ejecuciones del protocolo $(\Prover, \Verifier)$. Dentro de este protocolo, llamamos prueba \ZK a $\pi \in \ProofSpace$.
	Finalmente, decimos que $(\Prover, \Verifier)$ es no interactivo si hay un solo mensaje (la prueba $\pi$ enviada de $\Prover$ a $\Verifier$), y en caso contrario que es interactivo.
\end{definition}
...
En la imagen \ref{fig:interactive-zk-protocol}, el protocolo de ejemplo muestra una interacción de tres pasos entre $\Prover$ y $\Verifier$. La prueba $\pi$ está compuesta por todos los valores que $\Prover$ le envió a $\Verifier$ como respuesta a sus mensajes. En la mayoría de protocolos ZK, los valores que $\Verifier$ son en cierta medida aleatorios, es por eso que en esta figura elegimos nombrarlos como $r_i$.
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[font=\small]
		...
		% Output
		\node at (0,-7) {$\Prover(x,w) = (\pi_1, \pi_2, \pi_3)$};
		\node at (7,-7) {$\Verifier(x,\pi) \in \{0,1\}$};
...
\begin{itemize}[noitemsep]
	...
	\item Si un prover deshonesto $\Prover^*$ ejecuta su algoritmo con un $w$ tal que $(x,w) \not\in \Relation$, entonces probablemente $\Verifier$ podrá detectarlo. La probabilidad no es nula, pero es negligible. Qué tan baja queremos que sea esta probabilidad depende del nivel de bits de seguridad que exigimos a nuestro sistema criptográfico. Otra forma intuitiva de interpretar la solidez es que si $\Verifier$ fue convencido por $\Prover$, es porque probablemente este conocía un $w$ tal que $(x,w) \in \Relation$. Observamos que la propiedad de solidez, a diferencia de las otras, vale para \textbf{cualquier} prover.
	\item $\Verifier$ no aprende ninguna información sobre $w$ a partir de la prueba ZK $\pi$ (compuesta por los mensajes enviados por $\Prover$), sino que sólo aprende que $\Prover$ lo conoce. La propiedad de conocimiento cero se enuncia a partir de un simulador que no tiene acceso a un $w$ tal que $(x,w) \in \Relation$, pero tal que $\Verifier$ no puede distinguirlo (computacionalmente) de una interacción real con $\Prover$. Intuitivamente, la existencia de un simulador implica que todo lo que ve el \textit{verifier} durante el protocolo podría haberse generado sin conocer el \textit{witness}. Por lo tanto, la interacción no puede estar revelando información adicional sobre $w$. La clave es la \textbf{indistinguibilidad computacional}. Es crucial que el simulador sea eficiente, pues el tiempo sería otra forma de distinguir las interacciones.
	\item Un detalle adicional es que si bien la propiedad de solidez trata el caso donde un prover puede ser deshonesto, en ningún caso estamos tratando la posibilidad de que el verifier sea deshonesto y no acepte pruebas ZK válidas. Aquí es donde se observa \textit{honest-verifier}.
\end{itemize}

Desde el punto de vista de la teoría de la complejidad, los protocolos de pruebas \ZK están en la clase de complejidad \textit{Interactive Proofs} ($\IP$). Informalmente, una clase de lenguajes $L$ pertenece a $\IP$ si existe un protocolo interactivo $(\Prover, \Verifier)$ tal que para toda instancia $x \in \InstanceSpace$, el verifier $\Verifier$, que se ejecuta en tiempo polinomial, acepta con probabilidad 1 cuando $x \in L$ y rechaza con probabilidad al menos $1 - \epsilon$ cuando $x \notin L$. El prover $\Prover$ no está restringido computacionalmente, y la interacción puede contar con un número polinomial de rondas. Podemos pensar al lenguaje $L$ como inducido por $\Relation \subseteq \InstanceSpace \times \WitnessSpace$, donde $x \in L$ si y sólo si existe $w \in \WitnessSpace$ tal que $(x,w) \in \Relation$.
...
\begin{definition}[Esquema de cómputo verificable (\ZK)]
	...
	\begin{enumerate}[noitemsep]
		\item Completitud: Para $\pi = \Prover(x,y,w)$, vale $\Prob[\Verifier(x,y, \pi) = 1 \mid y = f(x,w)] = 1$.
		\item Solidez: Para $\pi = \Prover(x,y,w)$, vale $\Prob[\Verifier(x, y, \pi) = 1 \mid y \neq f(x,w)] \leq \epsilon$, donde $\epsilon$ es un valor negligible.
...
\begin{enumerate}[noitemsep]
	\item En una prueba, la propiedad de solidez del protocolo vale para todo $\Prover$, incluso con recursos computacionales no acotados. Es decir, la probabilidad de aceptación de una afirmación falsa está acotada por $\epsilon$ negligible independientemente del poder computacional de $\Prover$ (incluso no acotado).
	\item En un argumento, la propiedad de solidez del protocolo se garantiza únicamente asumiendo que $\Prover$ es eficiente. En particular, se toma que $\Prover$ sea probabilístico en tiempo polinomial. También reciben el nombre de \textit{Computationally Sound Proof System}, pero nosotros utilizaremos argumento. Si además vale la propiedad de conocimiento cero, diremos que es un argumento \ZK.
\end{enumerate}
