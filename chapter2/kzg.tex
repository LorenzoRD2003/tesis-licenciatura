...
Sea $m \in \N$ el número de puntos de evaluación distintos, y sean $z_1, \ldots, z_m \in \Fp$ los puntos de evaluación. Para $1 \leq j \leq m$, en $z_j$ queremos abrir $t_j$ polinomios $P_{j,0}, P_{j,1}, \ldots, P_{j,t_j-1} \in \poly{\Fp}_{\leq d}$ con compromisos
...
y enviar como argumento de apertura el compromiso $\pi_j := \Commit(W_j) = W_j(\tau)G_1 \in \GOne$, reduciendo el problema a $m$ aperturas \KZG ordinarias.
$$\pairing{C_j - \alpha_j G_1}{G_2} = \pairing{\pi_j}{(\tau - z_j)G_2}, \qquad 1 \leq j \leq m.$$
...
La segunda etapa de \GWCNineteen agrega estas $m$ ecuaciones de pairing en una sola. Para ello, el \textit{verifier} toma un segundo \textit{challenge} aleatorio $u \in \Fp$ y combina linealmente todas las instancias con potencias de $u$. Definimos
$$\Pi := \sum_{j=1}^{m} u^{j-1}\pi_j \in \GOne, \qquad E := \sum_{j=1}^{m} u^{j-1}\left(z_j\pi_j + C_j - \alpha_j G_1\right) \in \GOne.$$
Entonces, por bilinealidad,
$$\pairing{\Pi}{\tau G_2} = \prod_{j=1}^{m} \pairing{\pi_j}{\tau G_2}^{u^{j-1}} = \prod_{j=1}^{m} \pairing{z_j\pi_j + C_j - \alpha_j G_1}{G_2}^{u^{j-1}} = \pairing{E}{G_2}. $$
