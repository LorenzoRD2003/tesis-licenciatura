...
\begin{definition}[Transformación \FiatShamir]
	Sea $\Pi = (\Commit, \Open, \Verify)$ un \SigmaProtocol para una relación $\Relation \subseteq \InstanceSpace \times \WitnessSpace$, y supongamos que su espacio de desafíos es compatible con la salida de una función hash criptográfica $\Hash: \Strings{*} \to \Strings{n}$. La \emph{transformación \FiatShamir} es un protocolo no interactivo $\Pi_{FS} = (\Prove_{FS}, \Verify_{FS})$ definido como:
