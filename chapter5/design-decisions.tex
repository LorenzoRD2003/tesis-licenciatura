...
Durante este capítulo, utilizaremos $\ChainA$ para referirnos a la blockchain de origen y $\ChainB$ para referirnos a la blockchain destino.

\section{Resumen}

Este capítulo formaliza \emph{decisiones de diseño} para un \zkBridge sobre \Cardano, partiendo de las restricciones del modelo \eUTxO y de la capa de ejecución (límites \maxTxSize y \maxTxExUnits), discutidas en el Capítulo 4. Se propone separar el sistema en dos capas. Por un lado, una \emph{capa de actualización} que mantiene, en $\ChainB$, información verificable sobre $\ChainA$; y una \emph{capa de aplicación} que implementa la lógica de \textit{locking}/\textit{minting} y \textit{burning}/\textit{unlocking} sobre esa base. Esta estructura guía todas las decisiones.
...
\begin{itemize}[noitemsep]
	...
	\item Un \emph{oráculo de \StakeDistribution} actualizado por \Epoch y referenciable desde transacciones de mint (mediante reference inputs), evitando repetir certificados pesados en cada operación \cite{MithrilDocsCertChain}.
\end{itemize}
...
Debido a las limitaciones de \Cardano, en lugar de replicar explícitamente el estado de $\ChainA$, la capa de actualización debe basarse en \textit{commitments} o certificados criptográficos. En este trabajo, la capa de actualización se implementa conceptualmente mediante certificados de \emph{\StakeDistribution} provistos por \Mithril, que permiten mantener una referencia verificable y actualizada del estado de consenso de $\ChainA$ a lo largo del tiempo.
...
\begin{enumerate}[noitemsep]
	\item Los operadores actualizan periódicamente la representación del estado de $\ChainA$ en $\ChainB$ introduciendo nuevos certificados de \StakeDistribution.
	\item Un usuario realiza una transacción $\LockTx$ de \textit{locking} de activos en $\ChainA$.
	\item El usuario construye la evidencia necesaria para demostrar la validez de dicha operación, incluyendo certificados de \TransactionSnapshot que argumentan la inclusión de $\LockTx$ en $\ChainA$.
	\item El usuario realiza una transacción $\MintTx$ de \textit{minting} de activos en $\ChainB$, donde presenta la evidencia, apoyándose en la información mantenida por la capa de actualización.
...
El modelo \eUTxO favorece representar el estado de una aplicación como una colección de salidas consumibles y recreables, en lugar de un estado global mutable compartido \cite{Chakravarty2020}. En este enfoque, cada transición de estado se expresa como una transacción que consume una representación previa y produce una nueva, garantizando que las invariantes del sistema se mantengan localmente. En el contexto de un \zkBridge, esto implica que el estado relevante (por ejemplo, la información necesaria para validar eventos provenientes de $\ChainA$ en $\ChainB$) debe representarse de forma compacta y verificable, permitiendo su evolución a través de transiciones explícitas.
...
El diseño del sistema requiere que la actualización del estado global y la ejecución de las operaciones de aplicación (por ejemplo, \textit{minting} o actualización del conjunto de transacciones procesadas) ocurran de manera atómica. Esto garantiza que no existan ejecuciones parciales en las cuales una operación sea aplicada sin reflejarse en el estado, o viceversa. En el modelo \eUTxO, esta propiedad puede lograrse mediante la construcción de transacciones que incorporen simultáneamente la ejecución de múltiples \textit{scripts}, donde se valida que tienen dependencia mutua. De esta forma, se preserva la coherencia del sistema.
...
\begin{itemize}[noitemsep]
	...
	\item Un argumento de la pertenencia de una transacción a un \Snapshot de \Mithril.
\end{itemize}
...
En $\ChainB$, las transacciones principales consisten en la renovación de certificados de \Mithril por parte de los operadores para tener disponibilidad de información en la capa de actualización ($\UpdaterTx$), y el minteo de activos representativos por parte de los usuarios presentando la evidencia de inclusión de la transacción de locking en $\ChainA$ ($\MintTx$).

En un esquema bidireccional de \Bridge, el proceso se completa con una transacción de burning de activos en $\ChainB$ ($\BurnTx$) y la correspondiente transacción de unlocking de activos en $\ChainA$ ($\UnlockTx$), completando el ciclo de transferencia.
