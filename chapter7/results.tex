...
Este capítulo resume el resultado principal de la implementación final: el flujo transaccional de la Sección~\ref{sec:final-bridge-flow-flujo-final-del-zkbridge} entra dentro de los límites de recursos de \Cardano que motivaron el diseño. En particular, retomamos \maxTxSize y \maxTxExUnits, introducidos en la Sección~\ref{sec:motivation-limites-del-protocolo-y-su-importancia-en-un-zkbridge}. Para evaluarlo, tomamos las transacciones concretas del flujo final y comparamos su tamaño serializado, su consumo de CPU y su consumo de memoria contra los parámetros de protocolo de la Tabla~\ref{tab:cardano-protocol-limits}.
...
\begin{enumerate}[noitemsep]
  \item El flujo integrado del prototipo, cuya secuencia se detalla en la Subsección~\ref{subsec:final-bridge-flow-el-flujo-principal-del-zkbridge}, se ejecuta de punta a punta mediante \texttt{./scripts/bridge.sh run --strict}, encadenando construcción del bundle de \Mithril, \textit{preflight}, \texttt{aiken check} y corrida completa sobre un nodo local \Dolos.
  \item La suite de \texttt{aiken check}, descrita en la Subsección~\ref{subsec:final-bridge-flow-estrategia-de-tests-con-aiken-check}, cubre los validadores principales del sistema: \textit{minting}, \StakeDistribution, actualizador de transacciones usadas, pertenencia al \Snapshot, \texttt{phase1}, \texttt{phase2} y \textit{fixtures} de integración.
