...
En esta ruta \LegacyHashScheme, el mensaje efectivamente autenticado por el certificado es una Merkle Root $\TxSetRoot \in \Strings{256}$, la cual es la del \ProtocolMsg de las secciones anteriores \cite{MithrilSTMRepo}. El \TransactionSnapshot expuesto por la API del \MithrilAggregator agrega además un número de bloque certificado $\SnapshotBlockNumber$ y un identificador derivado
...
Si $\lambda(\tx)$ denota la codificación en bytes del hash hexadecimal de $\tx$, entonces la raíz local del rango $R_u$ es $\TxRangeRoot{u}$ del Merkle Tree $\mathcal{T}_{\mathcal{L}, \BLAKETwoSTwoFiveSix}$ con $L = \{\lambda(\tx_{u,i})\}_{1\leq i\leq m_u}$.
